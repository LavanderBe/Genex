...
public class CrudBudget implements ICrud<Budget> {
    ...
    public void addEntity(Budget b) {
        ...
        try (PreparedStatement pst = getCnx().prepareStatement(sql)) {
            ...
            pst.setString(2, b.getSponsorId());          // nullable — OK
    ...
    public void updateEntity(Budget b, String id) {
        String sql = "UPDATE budget SET center_id=?, sponsor_id=?, fiscal_year=?, " +
                     "allocated_amount=?, spent_amount=? WHERE id=?";
        try (PreparedStatement pst = getCnx().prepareStatement(sql)) {
            pst.setString(1, b.getCenterId());
    ...
    public void getEntity(Budget b) {
        String sql = "SELECT b.*, s.name AS sponsor_name, s.industry, s.contact_email, " +
    ...
    public List<Budget> getAll() {
        ...
        String sql = "SELECT b.*, s.name AS sponsor_name, s.industry, s.contact_email, " +
                     "s.logo_url, s.website_url " +
                     "FROM budget b LEFT JOIN sponsors s ON b.sponsor_id = s.id " +
